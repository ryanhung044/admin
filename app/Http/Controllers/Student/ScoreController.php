...
class ScoreController extends Controller
{
    ...
    public function bangDiem(Request $request)
    {
        try {
            $userCode = $request->user()->user_code;
            // $subjectAll = Subject::Where('subject_code', 'LIKE', 'ALL%')
            ...
            //     ];
            // });
            // Lấy danh sách các môn học chung (ALL)
            $subjectAll = Subject::where('subject_code', 'LIKE', 'ALL%')
                ->with('semester:cate_code,cate_name')
                ->get();
            ...
            $result = $listSubject->map(function ($user) use ($subjectAll) {
                $userSemesterCode = $user->semester_code;
                $subjectsMajor = $user->subjectMajor;
                $subjectsNarrowMajor = $user->subjectNarrowMajor;
                $scores = $user->scores;
                ...
                // Gộp tất cả các `subject` từ `subjectMajor`, `subjectNarrowMajor` và `subjectAll`
                $allSubjects = collect($subjectsMajor)
                    ->merge($subjectsNarrowMajor)
                    ->merge($subjectAll);
                ...
                foreach ($allSubjects as $subject) {
                    ...
                    if ($scoresMap->has($subjectCode)) {
                        ...
                        // Nếu không trùng, lấy thông tin từ `subjectMajor`, `subjectNarrowMajor`, hoặc `subjectAll`
                        $isPass = $subject->semester_code === $userSemesterCode ? 'Studying' : 'Notyet';
                        $finalSubjects[] = [
                            'semester_code' => $subject->semester->cate_code,
                            'semester_name' => $subject->semester->cate_name,
